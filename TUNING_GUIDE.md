...
### 4) Hardware limits (safety / resource caps)

This repo includes a preflight checker that blocks start/run tasks when the machine is likely to fail due to resource constraints (disk nearly full, low available RAM, runaway directory growth).

Run the checker (repo root):

```powershell
.venv/Scripts/python.exe scripts/hardware_limits_check.py --json
```

Tune thresholds in `config.json` under `hardware_limits`:
- Global minimums: `min_free_disk_percent`, `min_free_disk_bytes`, `min_avail_ram_bytes`
- Per-directory caps (bytes/files): `hardware_limits.dirs.temporary|active|holding|semantic|procedural|event`

Notes:
- The defaults use disk-relative sizing (`max_bytes_fraction_of_disk`) plus absolute caps (`max_bytes_cap`) so the limits adapt to different machines.
- If `psutil` is installed, available RAM is checked; otherwise the RAM check is best-effort skipped.
...
- Live runtime + supporting metrics: `http://127.0.0.1:8000/dashboard.html?autofetch=live`
- Analysis mode + supporting metrics: `http://127.0.0.1:8000/dashboard.html?autofetch=analysis`
- Workflow pies: `http://127.0.0.1:8000/composition_workflow_pies.html?autofetch=1`

If you open those pages from `file://` instead, the browser cannot auto-read the repo artifacts. Use the local server URLs above for true autofetch, or load files manually with the page pickers.

Agent-attach tip:
- A normal `python scripts\composition_workflow_chart_report.py` refresh now also rebuilds the compact pie payload automatically.
- Refresh the compact pie payload with `python scripts\composition_workflow_pie_live.py` only when you want to rebuild that smaller artifact by itself.
- Attach `TemporaryQueue\composition_workflow_pie_live.json` when you want the current workflow-pie state without the larger workflow-chart report payload.
