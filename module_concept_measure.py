...
def classify_measurement_adequacy_level(
    *,
    ...
    if not measurement_recorded:
        if skipped_events > 0 and completed_events == 0:
            return {
                "level": "skipped",
                "reason": "Recent telemetry shows skipped measurement attempts and no persisted measurement evidence in the current review surface.",
            }
        return {
            "level": "absent",
            "reason": "No persisted spatial measurement evidence was found in the current review surface.",
        }

    if supporting_signal_count >= 4:
        return {
            "level": "strong",
            "reason": "Measurement evidence is backed by multiple supporting surfaces: persisted records, snapshots or graph metrics, and telemetry or bridge evidence.",
        }
    if supporting_signal_count >= 2:
        return {
            "level": "partial",
            "reason": "Measurement evidence exists and is supported by some downstream surfaces, but not enough to treat adequacy as strong yet.",
        }
    return {
        "level": "weak",
        "reason": "Measurement evidence exists, but only a thin portion of the supporting snapshot, graph, bridge, or telemetry surfaces is present.",
    }
...
def attach_conceptual_measurement_to_relational_state(
    ...
    *,
    ...
    tick_id_hint: Optional[str] = None,
) -> Dict[str, Any]:
    ...
    # Build deterministic graph snapshot when relational data exists.
    try:
        tick_source = tick_id_hint
        if not isinstance(tick_source, str) or not tick_source:
            for candidate in (
                record.get("cycle_id"),
                rs.get("cycle_id") if isinstance(rs.get("cycle_id"), str) else None,
                record.get("tick_id"),
                record.get("id"),
            ):
                if isinstance(candidate, str) and candidate:
                    ...
                    break
        if not isinstance(tick_source, str) or not tick_source:
            tick_source = "concept_measure"

        from module_integration import build_graph_snapshot

        snapshot = build_graph_snapshot(rs, str(tick_source))
