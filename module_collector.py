...
def _determinism():
    """Return (deterministic_mode, fixed_timestamp)."""
...
def _format_output(module, status, output, summary=None, details=None):
    ...
    rec = {
        ...
        "timestamp": fixed_ts if (deterministic_mode and fixed_ts) else _now_ts(),
...
try:
    ...
    ts = fixed_ts if (deterministic_mode and fixed_ts) else datetime.fromtimestamp(time.time()).isoformat()
    print(json.dumps({{"module":"{module_name}","status":"completed","timestamp":ts,"summary":str(result)[:80],"details":result}}))
except Exception as e:
    ts = fixed_ts if (deterministic_mode and fixed_ts) else datetime.fromtimestamp(time.time()).isoformat()
    print(json.dumps({{"module":"{module_name}","status":"error","timestamp":ts,"summary":str(e)[:80],"details":str(e)}}))
"""
...
def collect_results(plan, data_id, content="demo content", timeout_seconds=None):
    ...
    for proc, module_name in processes:
        try:
            ...
            if stdout:
                decoded_stdout = _safe_decode(stdout)
                try:
                    item = json.loads(decoded_stdout.strip())
                    ...
                    if start_rec:
                        ...
                        item["start_ts"] = fixed_ts if (deterministic_mode and fixed_ts) else datetime.fromtimestamp(start_rec["t"]).isoformat()
                        item["end_ts"] = fixed_ts if (deterministic_mode and fixed_ts) else datetime.fromtimestamp(time.time()).isoformat()
                        # resource metrics
                    ...
                    results.append(_format_output("unknown", "parse_error", decoded_stdout))
            if stderr:
                results.append(_format_output("unknown", "stderr", _safe_decode(stderr)))
