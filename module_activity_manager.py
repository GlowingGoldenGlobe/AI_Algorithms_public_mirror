...
class ActivityTraceRow(TypedDict, total=False):
    activity_id: str
    activity_type: str
    targets: list[str]
    priority: float
...
def execute_activity(*, activity: Activity, modules: dict[str, Callable[[Activity], Any]]) -> Any:
    ...
    try:
        ...
        role = str(at) if at is not None else 'unknown'
        incr_counter('parameter_events_used_total', 1)
        incr_counter(f'parameter_events_used_by_role.{role}', 1)
    except Exception:
        pass
    return fn(activity)
    ...
    if isinstance(modules, dict):
        try:
            ...
            for k, v in modules.items():
                if k in ('__verifier__', 'verifier'):
                    continue
                if callable(v):
                    available_roles.append(str(k))
            if available_roles:
                incr_counter('parameter_events_available_total', len(available_roles))
                for role in available_roles:
                    incr_counter(f'parameter_events_available_by_role.{role}', 1)
        except Exception:
            pass
...
def _normalize_activity_trace_entry(activity: Any, status: str) -> Optional[ActivityTraceRow]:
    if not isinstance(activity, dict):
        return None
    aid = activity.get('activity_id')
    atype = activity.get('activity_type')
    if not isinstance(aid, str) or not aid or not isinstance(atype, str) or not atype:
        return None
    targets = activity.get('targets')
    target_list = [t for t in (targets or []) if isinstance(t, str) and t]
    try:
        priority = float(activity.get('priority') or 0.0)
    except Exception:
        priority = 0.0

    row: ActivityTraceRow = {
        'activity_id': aid,
        'activity_type': atype,
        'targets': target_list,
    ...
    meta = activity.get('metadata') if isinstance(activity.get('metadata'), dict) else None
    if isinstance(meta, dict):
        pf = meta.get('precondition_failures')
        if isinstance(pf, list):
            row['precondition_failures'] = [str(x) for x in pf if isinstance(x, str)]
        res = meta.get('result')
        if isinstance(res, dict):
            try:
                if isinstance(res.get('start_ts'), (int, float)):
                    row['start_ts'] = float(res.get('start_ts'))
                if isinstance(res.get('end_ts'), (int, float)):
                    row['end_ts'] = float(res.get('end_ts'))
            except Exception:
                pass
            ver = res.get('verification')
            if isinstance(ver, dict) and isinstance(ver.get('ok'), bool):
                row['verifier_ok'] = bool(ver.get('ok'))

    return row


def normalize_activity_queue_trace(*, queue: ActivityQueue, max_items: int = 50) -> dict[str, Any]:
    """Return a normalized ActivityQueue trace for decision logging."""
    out = {
        'schema_version': '1.0',
        'pending': [],
        'active': [],
    ...
    def _collect(rows: Any, status: str) -> list[ActivityTraceRow]:
        if not isinstance(rows, list):
            return []
        normalized: list[ActivityTraceRow] = []
        for a in rows:
            row = _normalize_activity_trace_entry(a, status)
            if row is not None:
                normalized.append(row)
        normalized.sort(key=lambda r: (str(r.get('activity_id') or ''), str(r.get('activity_type') or '')))
    ...
    pending = _collect(queue.get('pending'), 'pending')
    active = _collect(queue.get('active'), 'active')
    completed = _collect(queue.get('completed'), 'completed')
